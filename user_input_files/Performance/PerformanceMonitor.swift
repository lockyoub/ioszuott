...
class PerformanceMonitor: ObservableObject {
    ...
    private func getCurrentCPUUsage() -> Double {
        ...
        guard result == KERN_SUCCESS, let threads = threadsList, threadsCount > 0 else {
            // 如果获取失败，返回默认值
            return 0.0
        }
        ...
        // 第三步：正确释放内存 - 使用实际分配的大小
        if let threads = threadsList {
            let deallocateSize = vm_size_t(Int(threadsCount) * MemoryLayout<thread_t>.stride)
