...
// MARK: - NotificationManager扩展: 设备Token和远程通知处理
extension NotificationManager {
    
    /// 更新设备Token
    /// - Parameter token: 设备Token字符串
    func updateDeviceToken(_ token: String) {
        UserDefaults.standard.set(token, forKey: "deviceToken")
        print("设备Token已更新: \(token)")
    ...
    func handleRemoteNotification(_ userInfo: [AnyHashable: Any]) {
        ...
        if let type = userInfo["type"] as? String,
           ...
           let message = userInfo["message"] as? String {
            ...
            let notificationType = TradingNotificationType.from(string: type)
            let notification = TradingNotification(
                type: notificationType,
                title: title,
                message: message,
                stockCode: userInfo["stockCode"] as? String,
            ...
            // 发送本地通知事件
            NotificationCenter.default.post(
                name: .newTradingNotificationReceived,
                object: notification
            )
        }
    }
    ...
    func handleNotificationTap(_ userInfo: [AnyHashable: Any]) {
        ...
        if let type = userInfo["type"] as? String {
            ...
            case "order_status":
                ...
                NotificationCenter.default.post(
                    ...
                    object: userInfo["orderID"]
                )
            case "trade_execution":
                // 导航到交易记录页面
                NotificationCenter.default.post(
                    name: .navigateToTrades,
                    object: userInfo
                )
            case "risk_alert":
                // 导航到风险监控页面
                NotificationCenter.default.post(
                    name: .navigateToRiskMonitor,
                    object: userInfo
                )
            case "price_alert":
                // 导航到股票详情页面
            ...
            default:
                // 默认导航到通知列表
                NotificationCenter.default.post(
                    name: .navigateToNotificationList,
                    object: nil
                )
            }
        }
        ...
        if let notificationId = userInfo["notificationId"] as? String,
           ...
           let index = notifications.firstIndex(where: { $0.id == uuid }) {
            notifications[index].isRead = true
            if unreadCount > 0 {
                unreadCount -= 1
            }
    ...
    private func uploadDeviceTokenToServer(_ token: String) async {
        // 这里可以实现向服务器上传token的逻辑
        // 暂时只是打印日志
        print("向服务器上传设备Token: \(token)")
    }
}

// MARK: - TradingNotificationType扩展
extension TradingNotificationType {
    static func from(string: String) -> TradingNotificationType {
        switch string {
        case "order_status": return .orderStatus
        case "trade_execution": return .tradeExecution
        case "risk_alert": return .riskAlert
        case "price_alert": return .priceAlert
