...
class MarketDataService: ObservableObject {
    ...
    @Published var orderBookData: [String: LocalOrderBookData] = [:]
    ...
    private func updateOrderBookData(_ data: OrderBookDataMessage) async {
        let orderBookData = LocalOrderBookData(
            symbol: data.symbol,
            bidPrices: data.bidPrices,
            bidVolumes: data.bidVolumes,
            askPrices: data.askPrices,
...
// MARK: - 本地数据模型（用于WebSocket传输）

/// 本地订单簿数据模型（用于WebSocket传输，与SharedModels中的OrderBookData结构不同）
struct LocalOrderBookData {
    let symbol: String
