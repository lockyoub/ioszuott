...
extension EnhancedNetworkManager {
    ...
    func login(_ request: LoginRequest) async throws -> LoginResponse {
        ...
        return try await post("/api/v1/auth/login", body: body, type: LoginResponse.self)
    }
    ...
    func refreshToken(_ refreshToken: String) async throws -> LoginResponse {
        ...
        return try await post("/api/v1/auth/refresh", body: body, type: LoginResponse.self)
    }
    
    /// 【P1功能实现】撤销令牌
    func revokeToken(_ token: String, tokenType: String = "refresh_token") async throws {
        let body: [String: Any] = [
            "token": token,
            "token_type_hint": tokenType
        ]
        
        let _: [String: Any] = try await post("/api/v1/auth/revoke", body: body, type: [String: Any].self)
    }
    
    /// 获取当前用户信息
    func fetchCurrentUser() async throws -> UserInfo {
        return try await get("/api/v1/user/profile", type: UserInfo.self)
    }
}

// MARK: - NetworkManager单例支持
extension EnhancedNetworkManager {
